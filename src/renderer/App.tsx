...
import { Sparkles, Settings, LogOut, ChevronLeft, ChevronRight, LayoutDashboard, Wallet, ShieldAlert, X } from 'lucide-react';
import { oracleLogo, csfloatLogo, skinsLogo, dmarketLogo } from '../../assets/images';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.system) {
      ...
      if (window.electronAPI.system.onForceMaintenance) {
        unsubMaintenance = window.electronAPI.system.onForceMaintenance(() => {
          setSystemConfig((prev: any) => ({ ...prev, isMaintenanceMode: true }));
        });
      }
    }
    ...
    if (window.electronAPI?.auth) {
      ...
      window.electronAPI.auth.getStatus().then(({ isLoggedIn }) => {
        setIsLoggedIn(isLoggedIn);
        if (isLoggedIn && window.electronAPI?.balance) {
          window.electronAPI.balance.getBalance().then(res => {
    ...
    window.electronAPI?.settings?.getEncryptionStatus?.().then(({ isEncrypted }) => {
  ...
  if (isLoggedIn === null) {
    return (
      <div className="splash">
        <div className="splash-logo" style={{ background: 'transparent', width: 'auto', height: 'auto' }}>
          <img src={oracleLogo} alt="SkinOracle" style={{ width: 64, height: 64, objectFit: 'contain', filter: 'drop-shadow(0 4px 12px rgba(37, 99, 235, 0.4))' }} />
        </div>
  ...
  return (
    <HashRouter>
      ...
      {systemConfig?.globalBannerMessage && (
        <div style={{
          backgroundColor: systemConfig.globalBannerType === 'error' ? 'var(--so-error)' :
            systemConfig.globalBannerType === 'warning' ? 'var(--so-warning)' : 'var(--so-primary)',
          color: '#fff',
          padding: '8px 16px',
          textAlign: 'center',
          fontSize: '13px',
      ...
      {encryptionWarning && !encryptionWarningDismissed && (
        ...
        >
          ...
          <span style={{ fontSize: '12.5px', color: '#fde68a', fontWeight: 600, flex: 1 }}>
            OS encryption is unavailable on this system. Your API keys are stored as base64 (not hardware-protected).
            Avoid using SkinOracle on a shared or public machine.
          </span>
          <button
            onClick={() => {
      ...
      {systemConfig?.isMaintenanceMode ? (
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <nav style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                ...
                >
                  <img src={csfloatLogo} alt="CSFloat" style={{ width: 18, height: 18, objectFit: 'contain', flexShrink: 0 }} />
                ...
                >
                  <img src={dmarketLogo} alt="DMarket" style={{ width: 18, height: 18, objectFit: 'contain', flexShrink: 0 }} />
                ...
                */}
                ...
                <NavLink
                  to="/balance"
                  title={!isSidebarExpanded ? `Balance & Ledger (${userBalance || '$0.00'})` : undefined}
                  className={({ isActive }) => `sidebar-nav-item ${isActive ? 'active' : ''}`}
                  style={({ isActive }) => ({
                    display: 'flex',
                    alignItems: 'center',
                  ...
                  {({ isActive }) => (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <Wallet size={18} style={{ flexShrink: 0, color: isActive ? '#ffffff' : 'var(--so-text-secondary)' }} />
                      ...
                      {isSidebarExpanded && userBalance && (
                        <span
                          style={{
                            ...
                            padding: '2px 7px',
                            borderRadius: '10px',
                            backgroundColor: isActive ? 'rgba(255, 255, 255, 0.2)' : 'rgba(14, 165, 233, 0.12)',
        ...
        <Routes>
          <Route path="/register" element={<RegisterScreen onSuccess={() => setIsLoggedIn(false)} />} />
          <Route path="/verify" element={<OtpScreen onSuccess={() => setIsLoggedIn(true)} />} />
