...
export default function SkinscomWorkstation() {
  ...
  const fetchOrders = async () => {
    ...
    if (!tokenOk) {
      toast.error('Skins.com Session Token is not configured. Please set your token in Settings first.');
      return;
    }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {hasToken === false && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <KeyRound size={18} style={{ color: 'var(--so-warning)' }} /> Skins.com API key  is not configured. Please add your token in Settings before syncing orders.
          </div>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <img src={skinsLogo} alt="Skins.com" style={{ height: 26, width: 'auto', objectFit: 'contain' }} />
            <span style={{ color: 'var(--so-text-primary)', fontWeight: 800, fontSize: '16px', letterSpacing: '-0.3px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span
              style={{
                ...
                height: 7,
                borderRadius: '50%',
                backgroundColor: pricesLoaded ? 'var(--so-success)' : 'var(--so-warning)',
                display: 'inline-block',
              }}
            />
            <span style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.5px', color: pricesLoaded ? 'var(--so-success-text)' : 'var(--so-warning-text)' }}>
              {pricesLoaded
        ...
        <div className="stat-box" style={{ padding: '8px 16px', minWidth: '110px', textAlign: 'center' }}>
      ...
      <div className="card" style={{ margin: 0 }}>
        {orders.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--so-text-muted)' }}>
            <Gem size={36} style={{ marginBottom: '12px', opacity: 0.5 }} />
            <div style={{ fontWeight: 700, fontSize: '16px', color: 'var(--so-text-primary)', marginBottom: '6px' }}>
              No Skins.com buy orders loaded
            </div>
            <div style={{ fontSize: '13px' }}>
              Verify your Skins.com token in Settings, then click "Sync Orders" above.
            </div>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    />
                    <div>
                      <div style={{ fontWeight: 800, fontSize: '13.5px', color: 'var(--so-text-primary)' }}>{cleanTitle}</div>
                      <div style={{ fontSize: '11px', color: 'var(--so-text-muted)', display: 'flex', alignItems: 'center', gap: '8px', marginTop: '2px' }}>
                        {wear && <span style={{ color: 'var(--so-text-secondary)', fontWeight: 600 }}>{wear}</span>}
                  ...
                  <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '10px', color: 'var(--so-text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Accepted Price</div>
                      <div className="tabular-nums" style={{ fontSize: '15px', fontWeight: 800, color: 'var(--so-success-text)' }}>
                        {oracleData?.acceptedPrice ? `$${oracleData.acceptedPrice.toFixed(2)}` : '---'}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      title="Delete Order"
                    >
                      {deletingId === order.id ? <Loader2 size={14} className="spin" /> : <Trash2 size={14} />}
                    </button>
                  </div>
                </div>
              );
