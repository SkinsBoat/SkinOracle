...
export const CSFloatBuyLimitIndicator: React.FC<CSFloatBuyLimitIndicatorProps> = ({
  balance = 0,
  ...
  const activeOrdersCount = orders.reduce((sum, o) => sum + (o.qty || o.quantity || 1), 0);
  ...
  const percentage = maxLimitValue > 0 ? (totalProjectedValue / maxLimitValue) * 100 : 0;
  const activePct = maxLimitValue > 0 ? Math.min(100, (activeOrdersTotal / maxLimitValue) * 100) : 0;
  const selectedPct = maxLimitValue > 0 ? Math.min(100 - activePct, (previewSelectedTotal / maxLimitValue) * 100) : 0;

  // Over-limit excess
  const isValueExceeded = maxLimitValue > 0 ? totalProjectedValue > maxLimitValue : totalProjectedValue > 0;
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Gauge size={13} style={{ color: statusTheme.color, flexShrink: 0 }} />
          <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--so-text-secondary)', textTransform: 'uppercase', letterSpacing: '0.4px' }}>
          ...
          <span
            ...
            style={{
              ...
              padding: '1px 5px',
              borderRadius: '3px',
              backgroundColor: isCountExceeded ? 'rgba(239, 68, 68, 0.2)' : 'var(--so-surface-panel)',
              color: isCountExceeded ? '#ef4444' : 'var(--so-text-muted)',
              border: `1px solid ${isCountExceeded ? 'rgba(239, 68, 68, 0.4)' : 'var(--so-border-subtle)'}`,
            }}
          >
            {totalProjectedCount.toLocaleString()} / {maxLimitCount.toLocaleString()} orders
          </span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          >
            {isExceeded ? <AlertTriangle size={10} /> : <CheckCircle2 size={10} />}
            <span>{isZeroBalance ? 'Zero Balance ($0.00)' : `${statusTheme.label} (${percentage.toFixed(1)}%)`}</span>
          </span>
        </div>
      </div>
        ...
        {isExceeded && (
          <div
            style={{
              ...
              bottom: 0,
              width: '100%',
              background: 'linear-gradient(90deg, transparent 60%, rgba(239, 68, 68, 0.8) 100%)',
              pointerEvents: 'none',
            }}
          />
        )}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
          <span>
            Active: <strong className="tabular-nums" style={{ color: 'var(--so-text-primary)' }}>${activeOrdersTotal.toFixed(2)}</strong>
          </span>

          {previewSelectedTotal > 0 && (
            <span style={{ color: statusTheme.color }}>
              + Selected: <strong className="tabular-nums">+${previewSelectedTotal.toFixed(2)}</strong> ({previewSelectedCount})
            </span>
          )}

          {!isSoClose && selectedOrdersCount > 0 && (
            <span style={{ color: 'var(--so-primary)' }}>
              (Selected: <strong className="tabular-nums">${selectedOrdersTotal.toFixed(2)}</strong>)
            </span>
          )}
        </div>
