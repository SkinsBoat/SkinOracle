...
interface CSFloatSoCloseCardProps {
  ...
  onOpenLookup: (name: string, acceptedPrice?: number, currentMarketPrice?: number, iconUrl?: string) => void;
  wearShortcut: string;
}
    ...
    >
        ...
        {isSelected ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                onOpenLookup(item.name, item.acceptedPrice, item.currentMarketPrice, item.iconUrl);
              }}
              className="btn btn-sm"
      ...
      <div onClick={(e) => e.stopPropagation()}>
        <TrendSparkline
          ...
          height={32}
          onClick={() => onOpenLookup(item.name, item.acceptedPrice, item.currentMarketPrice, item.iconUrl)}
        />
      </div>
