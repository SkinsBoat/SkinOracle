...
interface CSFloatListingCardProps {
  ...
  onOpenLookup: (name: string, acceptedPrice?: number, currentListedPrice?: number, iconUrl?: string) => void;
  wearShortcut: string;
}
  ...
  return (
    ...
    >
        ...
        {isSelected ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                onOpenLookup(name, undefined, currentListedPriceDollar || undefined, item.icon_url);
              }}
              className="btn btn-sm"
      ...
      <div style={{ textAlign: 'center', minHeight: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', alignItems: 'center', marginTop: '2px' }}>
          {wearShortcut && (
            <span style={{ fontSize: '10px', color: 'var(--so-text-muted)', fontWeight: 700 }}>
              {wearShortcut}
            </span>
          )}
          {floatVal && (
            <span style={{ fontSize: '9.5px', color: 'var(--so-cyan-text)', fontFamily: 'monospace' }}>
              f: {floatVal}
            </span>
          )}
        </div>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
          ...
          <span className="tabular-nums" style={{ fontWeight: 800, color: currentListedPriceDollar ? 'var(--so-text-primary)' : 'var(--so-text-muted)' }}>
            {currentListedPriceDollar ? `$${currentListedPriceDollar.toFixed(2)}` : 'Not Listed'}
          </span>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ color: 'var(--so-text-muted)' }}>Target Listing</span>
          <span className="tabular-nums" style={{ fontWeight: 800, color: analysis?.targetListingPrice ? 'var(--so-success-text)' : 'var(--so-text-muted)' }}>
      ...
      <div style={{ display: 'flex', gap: '6px' }} onClick={(e) => e.stopPropagation()}>
        {!isListed ? (
          <button
            ...
            style={{ flex: 1, fontWeight: 700, fontSize: '10.5px', padding: '4px 6px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}
            title={analysis?.targetListingPrice ? `List at $${analysis.targetListingPrice.toFixed(2)} (${isPrivateMode ? 'Private' : 'Public'})` : 'No recommended listing price'}
          >
          ...
          <>
            <button
              onClick={() => onUpdateListing(item, analysis?.targetListingPrice || (currentListedPriceDollar || 0))}
              disabled={isProcessing || !analysis?.targetListingPrice}
