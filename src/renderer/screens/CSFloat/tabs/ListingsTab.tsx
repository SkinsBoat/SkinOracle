...
import { CSFloatListingCard, ListingAnalysis } from '../components/CSFloatListingCard';
...
interface ListingsTabProps {
  ...
  setSelectedListingItems: React.Dispatch<React.SetStateAction<Record<string, boolean>>>;
...
export const ListingsTab: React.FC<ListingsTabProps> = ({
  ...
  const overpricedCount = inventory.filter((i) => listingAnalysis[i.asset_id]?.isOverpriced).length;
  const underpricedCount = inventory.filter((i) => listingAnalysis[i.asset_id]?.isUnderpriced).length;
  const actionReqListingCount = overpricedCount + underpricedCount;
  const matchedListingCount = inventory.filter((i) => !!listingAnalysis[i.asset_id]?.targetListingPrice).length;
  ...
  const selectAllMatchedListings = () => {
    const next: Record<string, boolean> = {};
    inventory.forEach((i) => {
      if (listingAnalysis[i.asset_id]?.targetListingPrice) next[i.asset_id] = true;
    });
    setSelectedListingItems(next);
  };
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11.5px', fontWeight: 700 }}>
              <span style={{ color: 'var(--so-text-muted)' }}>
                Items: <strong style={{ color: 'var(--so-text-primary)' }}>{inventory.length}</strong>
              </span>
              <span style={{ color: 'var(--so-text-muted)' }}>
                Listed: <strong style={{ color: 'var(--so-success-text)' }}>{listedCount}</strong>
              </span>
              <span style={{ color: 'var(--so-text-muted)' }}>
                Unlisted: <strong style={{ color: 'var(--so-accent-cyan)' }}>{unlistedCount}</strong>
              </span>
              {overpricedCount > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button
              ...
              style={{ fontWeight: 800, fontSize: '12px', padding: '6px 14px', display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              {batchListingProcessing ? <Loader2 size={13} className="spin" /> : <Edit3 size={13} />}
            ...
            {inventory.map((item) => {
              ...
              return (
                <CSFloatListingCard
                  ...
                  onToggleSelect={() =>
                    setSelectedListingItems((prev) => ({ ...prev, [item.asset_id]: !prev[item.asset_id] }))
                  }
                  analysis={analysis}
