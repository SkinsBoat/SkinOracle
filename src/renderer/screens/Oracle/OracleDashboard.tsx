...
export default function OracleDashboard() {
  ...
  }>({ totalEvaluated: 0, soCloseCount: 0, highLiquidityCount: 0, isBatchEvaluating: false, lastBuiltAt: null, batchProgress: null });
  ...
  useEffect(() => {
    if (window.electronAPI?.skinsnipe) {
      window.electronAPI.skinsnipe.getCacheStatus().then((status: any) => {
        if (!status) return;
    ...
    if (window.electronAPI?.oracle) {
      window.electronAPI.oracle.getAcceptedPrices().then((res: any) => {
        if (res && res.itemCount > 0) {
          const builtAt = res.storedAt ? new Date(res.storedAt).toLocaleTimeString() : null;
      ...
      window.electronAPI.oracle.getListingPrices().then((res: any) => {
        if (res && res.itemCount > 0) {
          const builtAt = res.storedAt ? new Date(res.storedAt).toLocaleTimeString() : null;
  ...
  const passingFilterCount = useMemo(() => {
    ...
    return cachedItemNames.filter(name => passesSmartPreFilters(name, preFilters, fullCache?.[name])).length;
  ...
  const soloMarket = (marketId: SkinsnipeMarketId) => {
    ...
    const name = SKINSNIPE_AVAILABLE_MARKETS.find(m => m.id === marketId)?.name || marketId;
  ...
  const deselectAllMarkets = () => {
    resetDefaultMarkets();
    toast.success(`Reset selected markets to default preset (${DEFAULT_SELECTED_MARKETS.length} markets)`);
  };
  ...
  const buildAcceptedPrices = async () => {
    ...
    try {
      ...
      if (allItemNames.length === 0) {
        toast.error('No price cache found. Please fetch prices via Skinsnipe or load a JSON cache file first.');
        setEvaluatedSummary(prev => ({ ...prev, isBatchEvaluating: false, batchProgress: null }));
        return;
      }

      // Apply Smart Pre-Filters
      const filteredItemNames = allItemNames.filter(name => passesSmartPreFilters(name, preFilters, fullCache[name]));

      if (filteredItemNames.length === 0) {
        toast.error('0 items matched your smart pre-filters. Please broaden your wear or category filters.');
        setEvaluatedSummary(prev => ({ ...prev, isBatchEvaluating: false, batchProgress: null }));
        return;
      }
        ...
        if (evalRes?.results) {
          evalRes.results.forEach((r: any) => {
            if (r.oracle?.finalAcceptedPrice) {
              const acceptedPrice = roundToCsFloatStep(r.oracle.finalAcceptedPrice);
      ...
      if (totalEvaluated > 0) {
        toast.error(`Evaluation interrupted after ${totalEvaluated.toLocaleString()} items evaluated (${total.toLocaleString()} priced): ${err.message}. Prices were saved!`, { duration: 8000 });
      } else {
      ...
      if (activeBatchId) {
        try {
          const finishRes = await window.electronAPI.oracle.finishBatch(activeBatchId, totalEvaluated);
          if (finishRes && finishRes.refundedCents > 0) {
            toast(`Auto-refunded $${(finishRes.refundedCents / 100).toFixed(2)} for ${finishRes.unusedItems.toLocaleString()} unused items.`, { icon: '💰' });
          }
        } catch (finishErr) {
          console.warn('[OracleDashboard] Failed to finish batch cleanly:', finishErr);
        }
      }

      // 3. Guarantee: Always store and activate any items that were successfully processed
      if (Object.keys(acceptedPriceMap).length > 0) {
        const storeResult = await window.electronAPI.oracle.storeAcceptedPrices(acceptedPriceMap);
        const builtAt = storeResult?.storedAt ? new Date(storeResult.storedAt).toLocaleTimeString() : new Date().toLocaleTimeString();
  ...
  const buildListingPrices = async () => {
    if (cacheStatus.itemCount === 0) {
      toast.error('Price cache is empty. Please fetch prices from Skinsnipe or load a JSON cache file first.');
      return;
    }

    setListingSummary(prev => ({ ...prev, isBatchEvaluating: true }));
    const toastId = toast.loading(`Building suggested listing prices using ${listingStrategy.mode.toUpperCase()} strategy...`);
    ...
    try {
      ...
      const listingPriceMap: Record<string, { listingPrice: number; mode: string; offsetPercent: number; lowestPrice: number; averagePrice: number }> = {};
      let total = 0;

      for (const name of filteredItemNames) {
        const itemData = fullCache[name];
        const listings: { p?: number; price?: number }[] = itemData?.l || [];
        const prices = listings.map(l => l.p ?? l.price ?? 0).filter(p => p > 0);
      ...
      const storeResult = await window.electronAPI.oracle.storeListingPrices(listingPriceMap);
      const builtAt = storeResult?.storedAt ? new Date(storeResult.storedAt).toLocaleTimeString() : new Date().toLocaleTimeString();
      ...
      toast.success(`Computed listing prices for ${total.toLocaleString()} items using ${listingStrategy.mode.toUpperCase()} strategy!`, { id: toastId });
    } catch (err: any) {
      console.error('[Oracle Dashboard] Build Listing Prices error:', err);
      toast.error(`Build Listing Prices failed: ${err.message}`, { id: toastId });
      setListingSummary(prev => ({ ...prev, isBatchEvaluating: false }));
    }
  };
  ...
  const handleFetchPrices = async () => {
    ...
    const toastId = toast.loading(`Initiating live fetch from Skinsnipe for ${marketCount} selected market${marketCount > 1 ? 's' : ''}...`);

    try {
      const result = await window.electronAPI.skinsnipe.fetchPrices(activeSelected);
      setIsDemoCache(false);
      setCacheStatus({ itemCount: result.itemCount, isFetching: false, lastFetchedAt: result.fetchedAt });
      ...
      if (result.criticalError) {
        toast.error(`Fetch Stopped: ${result.criticalError}`, { id: toastId, duration: 8000 });
      } else if (result.aborted) {
        toast.error('Fetch process stopped by user.', { id: toastId });
      } else if (result.errorCount && result.errorCount > 0) {
        toast.success(`Completed with ${result.errorCount} error(s). Cached ${result.itemCount.toLocaleString()} items!`, { id: toastId });
      } else {
        toast.success(`Fetched prices for ${result.itemCount.toLocaleString()} items across ${marketCount} Skinsnipe markets!`, { id: toastId });
      }
    } catch (err: any) {
      toast.error(err.message || 'Failed to fetch prices from Skinsnipe', { id: toastId, duration: 8000 });
      setCacheStatus(prev => ({ ...prev, isFetching: false }));
    }
  };
  ...
  const handleUploadJsonCache = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const res = await (window.electronAPI.skinsnipe as any).loadCacheJson(content);
        setIsDemoCache(false);
        setCacheStatus({ itemCount: res.itemCount, isFetching: false, lastFetchedAt: res.fetchedAt });
        if (res.marketCounts) {
          setMarketCounts(res.marketCounts);
        }
        await loadCacheKeys();
        toast.success(`Loaded ${res.itemCount.toLocaleString()} items from JSON cache into local memory!`);
      } catch (err: any) {
  ...
  const handleLoadDemoCache = async (forceRefresh?: boolean) => {
    ...
    try {
      const res = await window.electronAPI.skinsnipe.loadDemoCache({ forceRefresh });
      setIsDemoCache(true);
      setCacheStatus({ itemCount: res.itemCount, isFetching: false, lastFetchedAt: res.fetchedAt });
      if (res.marketCounts) {
        setMarketCounts(res.marketCounts);
      ...
      toast.error(err.message || 'Failed to load demo price cache', { id: toastId });
    }
  };
  ...
  const handleLookupSingleItem = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const acceptedData: { map: Record<string, any>; itemCount: number; storedAt: string | null } =
        await window.electronAPI.oracle.getAcceptedPrices();
      
      let builtItem = acceptedData?.map?.[exactName] || acceptedData?.map?.[hashName];
      if (!builtItem) {
        const lower = exactName.toLowerCase();
        const matchKey = Object.keys(acceptedData?.map || {}).find(k => k.toLowerCase() === lower);
        if (matchKey) builtItem = acceptedData.map[matchKey];
      }

      const rawListings: { m: string; p: number; q?: number }[] = cachedItem?.l || [];

      if (builtItem) {
        const sortedPrices = rawListings.map(l => l.p).sort((a, b) => a - b);
        const lowestPrice = sortedPrices[0] || builtItem.acceptedPrice;
        const avgPrice = sortedPrices.length > 0 ? sortedPrices.reduce((a, b) => a + b, 0) / sortedPrices.length : builtItem.acceptedPrice;
        ...
        setResults([{ name: hashName, source: 'not_found', oracle: null, listings: [] }]);
        toast.error(`Item "${hashName}" is not in the Built Accepted Price list.`, { id: toastId });
      }
    } catch (err: any) {
  ...
  const canBuild = cacheStatus.itemCount > 0 && !evaluatedSummary.isBatchEvaluating;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--so-text-primary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Sparkles size={24} style={{ color: 'var(--so-primary)' }} /> Oracle Pricing Central
          </h1>
          <p style={{ fontSize: '13.5px', color: 'var(--so-text-secondary)', marginTop: '4px' }}>
            Centralized pricing engine & persistent market price cache manager for all connected trading workstations.
          </p>
        </div>
      </div>
      ...
      <Step1MarketCache
        ...
        isBatchEvaluating={evaluatedSummary.isBatchEvaluating || listingSummary.isBatchEvaluating}
