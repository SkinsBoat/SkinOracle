...
export const TradeMarketFilterCard: React.FC<TradeMarketFilterCardProps> = ({
  ...
  return (
    <div
      ...
      style={getTradeFilterCardStyle(checked, isHovered, accentColor)}
    >
      <div style={getTradeFilterCheckboxStyle(checked, accentColor)}>
        {checked && <Check size={10} style={styles.checkIconWhite} />}
      </div>

      <span>Hide Trade</span>

      <span style={styles.tradeFilterCountBadge}>
        ({tradeCount})
      </span>
    </div>
  );
...
export const MarketSelectionChip: React.FC<MarketSelectionChipProps> = ({
  ...
  return (
    <div
      ...
      style={getMarketChipStyle(isSelected, isTrade, isCyan, accentColor)}
    >
      <div style={getMarketChipCheckboxStyle(isSelected, isTrade, isCyan, accentColor)}>
        {isSelected && <Check size={10} style={styles.checkIconWhiteSmall} />}
      </div>

      <MarketLogo marketId={id} marketName={name} size={15} />

      <span style={styles.marketChipName}>
        {name}
      </span>
      ...
      {isTrade && (
        <span
          className="badge"
          style={styles.tradePlatformBadge}
          title="Trade / Steam platform: prices reflect marked-up virtual credit or Steam wallet fees"
        >
          <AlertTriangle size={10} style={styles.tradePlatformBadgeIcon} />
          TRADE
        </span>
      )}
      ...
      {marketCount > 0 && (
        <span style={getMarketChipCountBadgeStyle(isSelected, isTrade, isCyan, missingQtyCount)}>
...
export const MarketSelectionToolbar: React.FC<MarketSelectionToolbarProps> = ({
  ...
  return (
    <div style={styles.toolbarContainer}>
      <div style={styles.toolbarTitleGroup}>
        <div style={styles.toolbarTitle}>
          <Filter size={16} style={{ color: accentColor }} /> {title}
        </div>
        <span
          className={`badge ${badgeClassName}`}
          style={getToolbarBadgeStyle(badgeClassName, accentColor, badgeTextColor, badgeBorderColor)}
        >
...
export const CacheStatusInfo: React.FC<{
  ...
  return (
    <div style={{ ...styles.cacheStatusContainer, ...style }}>
      <span className="tabular-nums" style={styles.cacheStatusCount}>
      ...
      {lastFetchedAt && (
        <span
          ...
          style={styles.cacheStatusTimeAgo}
        >
          {" "}
...
function getTradeFilterCardStyle(
  checked: boolean,
  isHovered: boolean,
  accentColor: string,
): React.CSSProperties {
...
function getTradeFilterCheckboxStyle(
  checked: boolean,
  accentColor: string,
): React.CSSProperties {
  return {
...
function getMarketChipCheckboxStyle(
  isSelected: boolean,
  isTrade: boolean,
  isCyan: boolean,
  accentColor: string,
...
function getMarketChipCountBadgeStyle(
  isSelected: boolean,
  isTrade: boolean,
  isCyan: boolean,
  missingQtyCount: number,
