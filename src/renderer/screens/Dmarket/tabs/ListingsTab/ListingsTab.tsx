...
export interface ListingsTabProps {
  ...
  onOpenLookupModal: (name: string, targetPrice?: number, marketPrice?: number, iconUrl?: string) => void;
...
export const ListingsTab: React.FC<ListingsTabProps> = ({
  ...
  >('all');
  const [inventoryMarketFilter, setInventoryMarketFilter] = useState<'all' | 'dmarket' | 'steam'>('all');
  const [depositingAssetId, setDepositingAssetId] = useState<string | null>(null);
  ...
  const selectedSteamCount = useMemo(() => {
    return inventory.filter(i => selectedInventory[i.assetId] && !i.inMarket).length;
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      const items: DmarketInventoryItem[] = Array.isArray(res?.items) ? res.items : [];
      setInventory(items);
      const dmarketCount = items.filter(i => i.inMarket).length;
      const steamCount = items.filter(i => !i.inMarket).length;
      toast.success(`Loaded ${items.length} inventory items (${dmarketCount} on DMarket, ${steamCount} in Steam)`, { id: toastId });
    } catch (err: any) {
  ...
  const handleDepositItem = async (item: DmarketInventoryItem) => {
    const inGameId = item.inGameAssetId || item.attributes?.inGameAssetId || item.assetId;
    if (!inGameId) {
      toast.error('Missing in-game asset coordinates for Steam deposit');
      return;
    }
    setDepositingAssetId(item.assetId);
    const toastId = toast.loading(`Initiating DMarket deposit for "${item.title}"...`);
    try {
  ...
  const handleBatchDepositSteamItems = async () => {
    const steamItems = inventory.filter(i => selectedInventory[i.assetId] && !i.inMarket);
    ...
    const toastId = toast.loading(`Registering deposit for ${assetIds.length} item(s) to DMarket...`);
    try {
  ...
  const handleCreateOffer = async (item: DmarketInventoryItem, priceUsd: number) => {
    if (!item.inMarket) {
      toast.error(`"${item.title}" is in your Steam inventory. Click "Deposit to DMarket" first to transfer it before listing.`, { duration: 6000 });
      return;
    }
    ...
    const targetAssetId = item.assetId || (item as any).itemId || (item as any).id;
    ...
    try {
      ...
      setInventory(prev => prev.filter(i => i.assetId !== item.assetId && i.assetId !== targetAssetId));
  ...
  const handleBatchListInventoryAtOracle = async () => {
    ...
    if (dmarketSelected.length === 0) {
      if (steamSelected.length > 0) {
        toast.error(`${steamSelected.length} selected item(s) are in Steam inventory. Click "Deposit to DMarket" first.`, { duration: 6000 });
      } else {
    ...
    if (toList.length === 0) {
      toast.error('None of the selected DMarket items have matching Oracle listing prices. Load Oracle prices first.');
      return;
    }
  ...
  return (
    <>
            ...
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{
                  ...
                  padding: '3px 8px',
                  borderColor: '#3b82f6',
                  color: inventoryMarketFilter === 'steam' ? '#ffffff' : '#60a5fa',
                }}
              >
          ...
          {inventory.length === 0 ? (
            ...
            >
              {filteredInventory.map(item => {
                ...
                const wearText = match ? match[2] : (item.attributes?.exterior || item.attributes?.cs2?.exterior || item.extra?.exterior || '');
                const wearShortcut = getWearShortcut(wearText);
                const floatVal =
                    ...
                    <div style={{ display: 'flex', gap: '6px' }} onClick={e => e.stopPropagation()}>
                      {!item.inMarket ? (
                        <button
                          ...
                          disabled={isProcessing || depositingAssetId === item.assetId}
                          className="btn btn-secondary btn-sm"
                          style={{
                            flex: 1,
                            fontWeight: 700,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {selectedSteamCount > 0 && (
              ...
              >
                {batchDepositing ? <Loader2 size={13} className="spin" /> : <ArrowUpRight size={13} />}
      ...
      <EditOfferModal
        offer={editingOffer}
        targetListingPrice={editingOffer ? listingAnalysis[editingOffer.id]?.targetListingPrice : undefined}
