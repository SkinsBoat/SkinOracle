...
export interface SoCloseTabProps {
  hasKey: boolean;
  targets: any[];
  balanceData: { usdFormatted?: string; usdCents?: number; balance?: number } | null;
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    <button
                      onClick={e => {
                        e.stopPropagation();
                        onOpenLookupModal(item.name, item.acceptedPrice, item.currentMarketPrice, item.iconUrl);
                      }}
                      className="btn btn-sm"
                      style={{
                        padding: '3px 6px',
