...
interface EditTargetModalProps {
  ...
  onClose: () => void;
  onUpdate: (target: DmarketTargetItem, newPriceUsd: number, newAmount: number) => Promise<void>;
}
...
export const EditTargetModal: React.FC<EditTargetModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await onUpdate(target, numPrice, isNaN(numAmount) || numAmount < 1 ? 1 : numAmount);
      onClose();
    } catch {
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: '12px', color: 'var(--so-text-muted)', marginTop: '2px' }}>
            Current Target Price: ${(parseFloat(target.priceCents) / 100).toFixed(2)}
          </div>
        </div>
