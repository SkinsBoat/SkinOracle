...
export const ItemLookupModal: React.FC<ItemLookupModalProps> = ({
  ...
  const dmarketEntry = cacheListings.find((m: any) =>
    isMarketMatch(m.m, "dmarket"),
  );
  const resolvedDmarketPrice =
    item.marketPrice || (dmarketEntry?.p ? Number(dmarketEntry.p) : null);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {resolvedDmarketPrice
                ? `$${resolvedDmarketPrice.toFixed(2)}`
                : "—"}
            </div>
          </div>
