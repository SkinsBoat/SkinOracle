...
export default function VersionBlockedScreen({ gateState }: VersionBlockedScreenProps) {
    ...
    >
      <div
        style={{
          ...
          width: '100%',
          textAlign: 'center',
          boxShadow: '0 24px 48px rgba(0, 0, 0, 0.7), 0 0 32px rgba(244, 63, 94, 0.08)',
        }}
      >
        {/* Warning Icon Badge */}
        <div
        ...
        >
          {gateState.reason || 'This build of SkinOracle is outdated and cannot connect to pricing and trading services.'}
        </p>
        ...
        >
          ...
          {(updateState.status === 'checking' || isChecking) && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'center', padding: '10px 0' }}>
              <RefreshCw size={20} className="spin" style={{ color: 'var(--so-primary)' }} />
          ...
          {updateState.status === 'available' && !isChecking && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              >
                <Download size={16} />
                {isStartingDownload ? 'Starting Download...' : 'Download & Install Update'}
              </button>
            </div>
          )}
          ...
          {updateState.status === 'downloading' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: 800, color: 'var(--so-primary)' }}>
          ...
          {(updateState.status === 'error' || updateState.status === 'not-available' || updateState.status === 'idle') &&
            !isChecking && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {updateState.error ? (
                  <div
                  ...
                  <div style={{ fontSize: '13px', color: '#94a3b8' }}>
                    Automatic update check did not detect an automated package. Please check again or download manually.
                  </div>
                )}
