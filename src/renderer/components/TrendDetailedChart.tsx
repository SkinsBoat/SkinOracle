...
export const TrendDetailedChart: React.FC<TrendDetailedChartProps> = ({
  ...
  const effectiveMomentum = typeof momentum === 'number' && !isNaN(momentum) ? momentum * 100 : deltaPct;
  ...
  const domain = [Math.max(0, parseFloat((min - padding).toFixed(2))), parseFloat((max + padding).toFixed(2))];

  const chartData = validData.map((val, idx) => {
    const rawDate = rawLabels[idx] || '';
    // Format YYYY-MM-DD -> MM/DD for clean axis labeling
    const shortLabel = rawDate.length >= 10 ? `${rawDate.slice(5, 7)}/${rawDate.slice(8, 10)}` : rawDate || `D${idx + 1}`;
    return {
      index: idx,
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--so-text-primary)' }}>
        ...
        {validData.length >= 2 && (
          ...
          >
            {isUp ? '▲' : isDown ? '▼' : '●'}{' '}
            {effectiveMomentum >= 0 ? `+${effectiveMomentum.toFixed(1)}%` : `${effectiveMomentum.toFixed(1)}%`} (14D)
          </span>
        )}
      </div>
      ...
      {validData.length >= 2 ? (
        <div>
          ...
          >
            <span>
              Min: <strong style={{ color: 'var(--so-text-primary)' }}>${min.toFixed(2)}</strong>
            </span>
            <span>
              Max: <strong style={{ color: 'var(--so-text-primary)' }}>${max.toFixed(2)}</strong>
            </span>
            <span>
              Latest: <strong style={{ color: themeColor }}>${lastVal.toFixed(2)}</strong>
            </span>
            <span>
              Points: <strong style={{ color: 'var(--so-text-primary)' }}>{validData.length} Days</strong>
            </span>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 8, right: 12, left: 12, bottom: 2 }}>
                ...
                <XAxis
                  ...
                  tick={{ fill: 'var(--so-text-muted)', fontSize: 10, fontFamily: 'monospace' }}
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        ...
                        >
                          <div style={{ color: 'var(--so-text-muted)', marginBottom: '2px', fontFamily: 'monospace' }}>
                            {item.date}
                          </div>
                          <div style={{ fontWeight: 800, color: themeColor, fontSize: '13px', fontFamily: 'monospace' }}>
                ...
                <Area
                  ...
                  activeDot={{ r: 5, fill: themeColor, stroke: '#ffffff', strokeWidth: 2 }}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <div style={{ textAlign: 'center', padding: '16px 10px', color: 'var(--so-text-muted)', fontSize: '11.5px' }}>
          <p style={{ margin: '0 0 4px 0', fontWeight: 600 }}>No 14-day historical trend recorded yet for this skin</p>
          <p style={{ margin: 0, fontSize: '10.5px', opacity: 0.7 }}>
            Snapshots accumulate automatically each day or can be generated via "Seed 14-Day History" in Oracle Dashboard.
          </p>
        </div>
      )}
    </div>
