...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  auth: {
    ...
    verify: (email: string, code: string) => safeInvoke('auth:verify', email, code),
  ...
  settings: {
    ...
    getEncryptionStatus: () => safeInvoke<{ isEncrypted: boolean }>('settings:get-encryption-status'),
  },

  // ── Skinsnipe (fetch prices using trader's own key, from trader's device) ──
  skinsnipe: {
    fetchPrices: (targetMarkets?: string[]) => safeInvoke('skinsnipe:fetch-prices', targetMarkets),
  ...
  oracle: {
    ...
    storeListingPrices: (map: Record<string, { listingPrice: number; mode: string; offsetPercent: number; lowestPrice: number; averagePrice: number }>) =>
  ...
  csfloat: {
    ...
    getOrders: (page?: number, limit?: number) => safeInvoke('csfloat:get-orders', page, limit),
    createBuyOrder: (marketHashName: string, maxPrice: number, quantity: number) =>
      safeInvoke('csfloat:create-buy-order', marketHashName, maxPrice, quantity),
    updateOrder: (orderId: string, ...args: any[]) =>
      safeInvoke('csfloat:update-order', orderId, ...args),
    deleteOrder: (orderId: string) => safeInvoke('csfloat:delete-order', orderId),
    getInventory: () => safeInvoke('csfloat:get-inventory'),
    createListing: (assetId: string, priceCents: number, privateMode: boolean) =>
      safeInvoke('csfloat:create-listing', assetId, priceCents, privateMode),
    deleteListing: (listingId: string) => safeInvoke('csfloat:delete-listing', listingId),
    updateListing: (listingId: string, priceCents: number, privateMode?: boolean) =>
  ...
  skinscom: {
    ...
    deleteOrder: (orderId: string) => safeInvoke('skinscom:delete-order', orderId),
  },
  ...
  dmarket: {
    ...
    createTarget: (title: string, priceInUsd: number, amount?: number, attrs?: any) =>
      safeInvoke('dmarket:create-target', title, priceInUsd, amount, attrs),
    deleteTarget: (targetId: string) => safeInvoke('dmarket:delete-target', targetId),
    updateTarget: (oldTargetId: string, title: string, newPriceInUsd: number, amount?: number, attrs?: any) =>
      safeInvoke('dmarket:update-target', oldTargetId, title, newPriceInUsd, amount, attrs),
    ...
    createOffers: (requests: any[]) => safeInvoke('dmarket:create-offers', requests),
    updateOffers: (requests: any[]) => safeInvoke('dmarket:update-offers', requests),
