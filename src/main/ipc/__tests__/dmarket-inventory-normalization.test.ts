...
describe('DMarket Inventory & Offer Normalization Engine', () => {
  describe('resolveDmarketTitle', () => {
    it('should resolve title from standard title property', () => {
      expect(resolveDmarketTitle({ title: 'AK-47 | Redline (Field-Tested)' })).toBe(
        'AK-47 | Redline (Field-Tested)',
      );
    });

    it('should resolve title from PascalCase Title (as in closed targets / buy order history)', () => {
      expect(resolveDmarketTitle({ Title: 'AWP | Asiimov (Field-Tested)' })).toBe(
        'AWP | Asiimov (Field-Tested)',
      );
    });

    it('should resolve title from marketHashName / MarketHashName / market_hash_name', () => {
      expect(resolveDmarketTitle({ marketHashName: 'M4A4 | The Emperor (Factory New)' })).toBe(
        'M4A4 | The Emperor (Factory New)',
      );
      expect(resolveDmarketTitle({ MarketHashName: 'USP-S | Printstream (Minimal Wear)' })).toBe(
        'USP-S | Printstream (Minimal Wear)',
      );
      expect(resolveDmarketTitle({ market_hash_name: 'Glock-18 | Water Elemental (Field-Tested)' })).toBe(
        'Glock-18 | Water Elemental (Field-Tested)',
      );
    });

    it('should resolve title from assetTitle, extra.name, or attributes.title', () => {
      expect(resolveDmarketTitle({ assetTitle: 'Desert Eagle | Blaze (Factory New)' })).toBe(
        'Desert Eagle | Blaze (Factory New)',
      );
      expect(resolveDmarketTitle({ extra: { name: 'MP9 | Mount Fuji (Minimal Wear)' } })).toBe(
        'MP9 | Mount Fuji (Minimal Wear)',
      );
      expect(resolveDmarketTitle({ attributes: { title: 'SSG 08 | Dragonfire (Field-Tested)' } })).toBe(
    ...
    it('should reconstruct wear condition in parentheses if missing from title but in attributes', () => {
      ...
      expect(resolveDmarketTitle(itemWithExt)).toBe('AK-47 | Slate (Field-Tested)');

      const itemWithFn = {
        Title: 'M4A1-S | Cyrex',
        attributes: { cs2: { exterior: 'exterior_factory_new' } },
      };
      expect(resolveDmarketTitle(itemWithFn)).toBe('M4A1-S | Cyrex (Factory New)');
    });
    ...
    it('should reconstruct StatTrak prefix if flagged in attributes', () => {
      ...
      expect(resolveDmarketTitle(stItem)).toBe('StatTrak™ AK-47 | Cartel (Field-Tested)');
    });
  ...
  describe('resolveDmarketImageUrl', () => {
    it('should preserve full http/https URLs', () => {
      const url = 'https://steamcommunity-a.akamaihd.net/economy/image/example.png';
    ...
    it('should correctly prefix relative Steam CDN economy image hashes', () => {
      const hash = '-9a81dlWLwJ2UUGcVs_nsVtzdOEdtWwKGZZLQHTxDZ7I56KU0Zwwo4NUX4oFJZEHLbXH5Ape';
      ...
      expect(resolveDmarketImageUrl({ attributes: { image: hash } })).toBe(expected);
    });
  ...
  describe('formatAsUuid & findDmarketUuid', () => {
    it('should validate and format standard hyphenated UUIDs', () => {
      ...
      expect(formatAsUuid('  F47AC10B-58CC-4372-A567-0E02B2C3D479  ')).toBe(validUuid);
    });
    ...
    it('should find UUID across top-level and nested properties', () => {
      ...
      expect(findDmarketUuid({ assetId: '45641473210', itemId: uuid })).toBe(uuid);
    });
    ...
    it('should NEVER return owner or depositor user account UUIDs as item UUID', () => {
      ...
      expect(resolveDmarketAssetId(steamItem)).toBe('8808581868:7993040398:53564593299:730');
    });
  ...
  describe('resolveDmarketAssetId', () => {
    ...
    it('should resolve assetId across various naming conventions when no UUID is present', () => {
      ...
      expect(resolveDmarketAssetId({ attributes: { inGameAssetId: '99999' } })).toBe('99999');
    });
  });
  ...
  describe('resolveDmarketPriceCents', () => {
    it('should resolve price in cents from different DMarket price objects', () => {
      ...
      expect(resolveDmarketPriceCents({ price: { amount: '12.50' } })).toBe(1250);
  ...
  describe('getItemListingPriceWithMap & getTradeTitle', () => {
    ...
    it('getTradeTitle should properly resolve title and wear condition', () => {
      expect(getTradeTitle({ Title: 'AK-47 | Redline (Field-Tested)' })).toBe('AK-47 | Redline (Field-Tested)');
      expect(getTradeTitle({ title: 'M4A4 | Buzz Kill', attributes: { exterior: 'Minimal Wear' } })).toBe(
