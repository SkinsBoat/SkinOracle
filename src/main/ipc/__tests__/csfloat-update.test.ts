...
describe('CSFloat Buy Order Update Payload Builder', () => {
  it('should construct valid payload with price in cents and default quantity 1', () => {
    ...
    expect(result.targetUrl).toBe('https://csfloat.com/api/v1/buy-orders/1015495269460083062');
  ...
  it('should preserve extra attributes like hybrid_properties (min_float, max_float)', () => {
    ...
    const result = buildUpdateBuyOrderPayload('1015495269460083062', 235, 2, extraProps);
    expect(result.payload).toEqual({
      hybrid_properties: {
        min_float: 0.07,
        max_float: 0.122,
  ...
  it('should parse string number inputs accurately', () => {
    const result = buildUpdateBuyOrderPayload('1015495269460083062', '450', '3');
    expect(result.payload).toEqual({
      max_price: 450,
      quantity: 3,
    });
  ...
  it('should throw error for invalid orderId', () => {
    expect(() => buildUpdateBuyOrderPayload('', 235)).toThrow('Valid orderId string is required');
  });

  it('should throw error for invalid or non-positive maxPriceCents', () => {
    expect(() => buildUpdateBuyOrderPayload('123', -50)).toThrow('Invalid maxPriceCents value');
    expect(() => buildUpdateBuyOrderPayload('123', 'invalid')).toThrow('Invalid maxPriceCents value');
  });
