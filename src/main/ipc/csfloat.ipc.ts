...
ipcMain.handle('csfloat:get-orders', async () => {
  ...
  console.log('[CSFloat IPC] Fetching ALL active buy orders across all pages...');
  ...
  while (hasMorePages) {
    console.log(`[CSFloat IPC] Fetching page ${currentPage} (limit=${pageSize})...`);
  ...
  console.log(`[CSFloat IPC] ✅ Fetched total ${allOrders.length} active buy orders across ${currentPage + 1} page(s).`);
  return { orders: allOrders, count: allOrders.length };
});


ipcMain.handle('csfloat:create-buy-order', async (_, marketHashName: string, maxPriceCents: number, quantity: number) => {
  const apiKey = secureGet(STORAGE_KEYS.CSFLOAT);
  if (!apiKey) throw new Error('CSFloat API key not set');

  const validPriceCents = snapCsFloatBuyOrderPriceCents(maxPriceCents);
  console.log(`[CSFloat IPC] create-buy-order: ${marketHashName}, raw: ${maxPriceCents}, snapped: ${validPriceCents}, qty: ${quantity}`);

  const res = await axios.post(
    CSFLOAT_BUY_ORDERS,
    { market_hash_name: marketHashName, max_price: validPriceCents, quantity },
    { headers: getHeaders(apiKey) },
  );
  return res.data;
});
...
ipcMain.handle('csfloat:update-order', async (_, orderId: string, maxPriceCentsArg: any, quantityArg?: any, extraProps?: any) => {
  const apiKey = secureGet(STORAGE_KEYS.CSFLOAT);
  if (!apiKey) throw new Error('CSFloat API key not set');
  ...
  const rawPriceCents = typeof maxPriceCentsArg === 'number' ? maxPriceCentsArg : parseFloat(maxPriceCentsArg);
  const quantity = quantityArg !== undefined ? (typeof quantityArg === 'number' ? quantityArg : parseInt(quantityArg, 10)) : 1;

  console.log(`[CSFloat IPC] update-order called with args:`, { orderId, maxPriceCents: rawPriceCents, quantity, extraProps });

  if (isNaN(rawPriceCents) || rawPriceCents <= 0) {
    throw new Error(`CSFloat update error: Invalid maxPriceCents value: ${maxPriceCentsArg}`);
  }
  ...
  try {
    ...
    console.log(`[CSFloat IPC] ✅ CSFloat Response Success (${res.status}):`, JSON.stringify(res.data, null, 2));
    return res.data;
  } catch (err: any) {
    const errorData = err.response?.data || err.message;
    console.error(`[CSFloat IPC] ❌ CSFloat Response Error (${err.response?.status || 'Network'}):`, JSON.stringify(errorData, null, 2));
    throw new Error(errorData?.message || errorData?.error || (typeof errorData === 'string' ? errorData : 'CSFloat API Update Failed'));
  }
});
...
ipcMain.handle('csfloat:get-me', async () => {
  ...
  console.log('[CSFloat IPC] /me Response data:', JSON.stringify(res.data, null, 2));
  return res.data;
});
...
ipcMain.handle('csfloat:get-inventory', async () => {
  ...
  try {
    ...
    const inventory = Array.isArray(res.data)
      ...
      ? res.data.inventory
      : [];
    console.log(`[CSFloat IPC] ✅ Fetched ${inventory.length} inventory items.`);
    return inventory;
  } catch (err: any) {
    const errorData = err.response?.data || err.message;
    console.error('[CSFloat IPC] ❌ Error fetching inventory:', errorData);
    throw new Error(errorData?.message || errorData?.error || (typeof errorData === 'string' ? errorData : 'Failed to fetch CSFloat inventory'));
  }
});

ipcMain.handle('csfloat:create-listing', async (_, assetId: string, priceCentsArg: any, privateMode: boolean = false) => {

  const apiKey = secureGet(STORAGE_KEYS.CSFLOAT);
  if (!apiKey) throw new Error('CSFloat API key not set');

  if (!assetId || typeof assetId !== 'string') {
    throw new Error('CSFloat create listing error: Valid assetId string is required');
  }

  const priceCents = typeof priceCentsArg === 'number' ? priceCentsArg : parseFloat(priceCentsArg);
  if (isNaN(priceCents) || priceCents <= 0) {
    throw new Error(`CSFloat create listing error: Invalid priceCents value: ${priceCentsArg}`);
  }
  ...
  try {
    ...
    throw new Error(errorData?.message || errorData?.error || (typeof errorData === 'string' ? errorData : 'Failed to create CSFloat listing'));
  }
});
...
ipcMain.handle('csfloat:delete-listing', async (_, listingId: string) => {
  ...
  if (!listingId || typeof listingId !== 'string') {
    throw new Error('CSFloat delete listing error: Valid listingId string is required');
  }
  ...
  try {
    ...
    const errorData = err.response?.data || err.message;
    console.error(`[CSFloat IPC] ❌ Error deleting listing ${listingId}:`, errorData);
    throw new Error(errorData?.message || errorData?.error || (typeof errorData === 'string' ? errorData : 'Failed to delete CSFloat listing'));
  }
});

ipcMain.handle('csfloat:update-listing', async (_, listingId: string, priceCentsArg: any, privateMode?: boolean) => {
  const apiKey = secureGet(STORAGE_KEYS.CSFLOAT);
  if (!apiKey) throw new Error('CSFloat API key not set');

  if (!listingId || typeof listingId !== 'string') {
    throw new Error('CSFloat update listing error: Valid listingId string is required');
  }

  const priceCents = typeof priceCentsArg === 'number' ? priceCentsArg : parseFloat(priceCentsArg);
  if (isNaN(priceCents) || priceCents <= 0) {
    throw new Error(`CSFloat update listing error: Invalid priceCents value: ${priceCentsArg}`);
  }
  ...
  try {
    ...
    console.log(`[CSFloat IPC] ✅ Listing ${listingId} updated successfully:`, res.data);
    return res.data;
  } catch (err: any) {
    const errorData = err.response?.data || err.message;
    console.error(`[CSFloat IPC] ❌ Error updating listing ${listingId}:`, errorData);
    throw new Error(errorData?.message || errorData?.error || (typeof errorData === 'string' ? errorData : 'Failed to update CSFloat listing'));
  }
});
