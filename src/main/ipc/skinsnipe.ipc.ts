...
type PriceCache = Record<string, { n: string; l: { m: string; p: number; q?: number }[] }>;
...
function parseSkinsnipeError(err: any): { statusCode: number | null; message: string; isCritical: boolean } {
  ...
  switch (status) {
    ...
    case 403:
      return {
        statusCode: 403,
        message: '❌ 403 Forbidden: User does not have access to this specific endpoint.',
        isCritical: true,
      };
    case 404:
      return {
    ...
    case 406:
      return {
        statusCode: 406,
        message: '❌ 406 Not Acceptable: Missing "Accept" and/or "Accept-Encoding" headers.',
        isCritical: true,
      };
    case 429:
      return {
        statusCode: 429,
        message: '❌ 429 Too Many Requests: Rate limit exceeded (calls per minute limit reached).',
        isCritical: true,
      };
    case 500:
      return {
        statusCode: 500,
        message: '❌ 500 Internal Server Error: Skinsnipe servers down or under maintenance.',
        isCritical: true,
      };
    default:
      return {
...
async function mergeAndBuild(event: IpcMainInvokeEvent, apiKey: string, targetMarkets?: string[]) {
  cancelRequested = false;
  const tempCache: PriceCache = {};
  const activeMarkets = targetMarkets && targetMarkets.length > 0 ? targetMarkets : DEFAULT_MARKETS;
  const failedMarkets: Array<{ market: string; error: string; statusCode: number | null }> = [];
  ...
  for (let i = 0; i < activeMarkets.length; i++) {
    ...
    try {
      ...
      console.error(`[Skinsnipe] Failed to fetch ${market} (HTTP ${parsedErr.statusCode}): ${parsedErr.message}`);
      failedMarkets.push({ market, error: parsedErr.message, statusCode: parsedErr.statusCode });
  ...
  if (!criticalError && !cancelRequested) {
    sendProgress(activeMarkets[activeMarkets.length - 1], activeMarkets.length, 'completed');
  }
...
ipcMain.handle('skinsnipe:fetch-prices', async (event, targetMarkets?: string[]) => {
  const apiKey = secureGet(STORAGE_KEYS.SKINSNIPE);
  if (!apiKey) throw new Error('Skinsnipe API key not set. Go to Settings to add it.');
...
ipcMain.handle('skinsnipe:load-cache-json', async (_, jsonContent: string) => {
  try {
    ...
    let cacheData = parsed.data?.priceCache || parsed.priceCache || (parsed.data && typeof parsed.data === 'object' && !Array.isArray(parsed.data) ? parsed.data : parsed);
...
ipcMain.handle('skinsnipe:load-demo-cache', async (_event: IpcMainInvokeEvent, options?: { forceRefresh?: boolean }) => {
  try {
    const localCachePath = path.join(app.getPath('userData'), 'demo-prices-cache.json');
    let cacheData: any = null;
    let source: 'local_cache' | 'cloud_download' | 'cloud_refreshed' = 'local_cache';
    ...
    if (!options?.forceRefresh && fs.existsSync(localCachePath)) {
      try {
        ...
        console.log(`[Skinsnipe] Loaded demo price cache from local disk: ${localCachePath}`);
      } catch (readErr: any) {
        console.warn('[Skinsnipe] Failed reading local demo cache, will re-fetch from cloud:', readErr.message);
        cacheData = null;
      }
    }
    ...
    if (!cacheData) {
      console.log('[Skinsnipe] Downloading demo price dataset from SaaS API (/oracle/demo-cache)...');
      try {
        ...
        try {
          fs.writeFileSync(localCachePath, JSON.stringify(cacheData), 'utf-8');
          console.log(`[Skinsnipe] Successfully cached demo price dataset to: ${localCachePath}`);
        } catch (saveErr: any) {
          console.warn('[Skinsnipe] Could not persist demo cache to disk:', saveErr.message);
        }
      } catch (networkErr: any) {
        // Fallback: If network failed but an existing local cache is on disk
        if (fs.existsSync(localCachePath)) {
          console.warn('[Skinsnipe] Network fetch failed, falling back to local cache file on disk:', networkErr.message);
          const jsonContent = fs.readFileSync(localCachePath, 'utf-8');
          const parsed = JSON.parse(jsonContent);
          cacheData = parsed.data?.priceCache || parsed.priceCache || parsed;
          source = 'local_cache';
