...
describe('TrendStore SQLite Service', () => {
  ...
  it('saves and retrieves daily snapshots across multiple dates', async () => {
    const cacheDay1 = {
      'AK-47 | Redline (Field-Tested)': {
        ...
        l: [{ m: 'csfloat', p: 12.50 }, { m: 'skinscom', p: 12.60 }],
      },
      'AWP | Asiimov (Field-Tested)': {
        n: 'AWP | Asiimov (Field-Tested)',
        l: [{ m: 'csfloat', p: 95.00 }, { m: 'skinscom', p: 96.00 }],
      },
    };
    ...
    const cacheDay2 = {
      'AK-47 | Redline (Field-Tested)': {
        n: 'AK-47 | Redline (Field-Tested)',
        l: [{ m: 'csfloat', p: 13.00 }, { m: 'skinscom', p: 13.10 }],
      },
      'AWP | Asiimov (Field-Tested)': {
        n: 'AWP | Asiimov (Field-Tested)',
        l: [{ m: 'csfloat', p: 94.00 }, { m: 'skinscom', p: 93.50 }],
      },
    };
    ...
    const trends = await store.getTrendHistoryBatch(['AK-47 | Redline (Field-Tested)'], 14);
  ...
  it('seeds 14-day mock trend history and clears database on demand', async () => {
    const cache = {
      'AK-47 | Slate (Field-Tested)': { l: [{ m: 'csfloat', p: 8.50 }, { m: 'skinscom', p: 8.60 }] },
      'M4A4 | Howl (Factory New)': { l: [{ m: 'csfloat', p: 4500.00 }] },
      'Glock-18 | Fade (Factory New)': { l: [{ m: 'csfloat', p: 1200.00 }] },
      'USP-S | Printstream (Field-Tested)': { l: [{ m: 'csfloat', p: 45.00 }] },
    };
    ...
    const trends = await store.getTrendHistoryBatch(['AK-47 | Slate (Field-Tested)'], 14);
    expect(trends['AK-47 | Slate (Field-Tested)'].labels.length).toBe(14);
    expect(trends['AK-47 | Slate (Field-Tested)'].overallAverages.length).toBe(14);
